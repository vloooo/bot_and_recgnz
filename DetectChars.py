...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:  # for each possible plate, this is a big for loop that takes up most of the function
        ...
        possiblePlate.imgGrayscale, possiblePlate.imgThresh, possiblePlate.imgThreshForRcgnz = Preprocess.preprocessS(
        ...
        possiblePlate.imgThreshForRcgnz = cv2.resize(possiblePlate.imgThreshForRcgnz, (0, 0), fx=1.6, fy=1.6,
                                                     interpolation=cv2.INTER_LINEAR)

        thresholdValue, possiblePlate.imgThresh = cv2.threshold(possiblePlate.imgThresh, 0.0, 255.0,
                                                                cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        thresholdValue, possiblePlate.imgThreshForRcgnz = cv2.threshold(possiblePlate.imgThreshForRcgnz, 0.0, 255.0,
                                                                        cv2.THRESH_BINARY | cv2.THRESH_OTSU)
        ...
        possiblePlate.strChars = recognizeCharsInPlate(possiblePlate.imgThreshForRcgnz,
...
def findListOfListsOfMatchingChars_Char(listOfPossibleChars):
    listOfListsOfMatchingChars = []  # this will be the return value
    # print("Now we check which contours are similar")
    ...
    for possibleChar in listOfPossibleChars:  # for each possible char in the one big list of chars
        ...
        listOfMatchingChars = findListOfMatchingChars_Char(possibleChar,
        ...
        listOfListsOfMatchingChars.append(listOfMatchingChars)
        listOfPossibleCharsWithCurrentMatchesRemoved = list(set(listOfPossibleChars) - set(listOfMatchingChars))
        recursiveListOfListsOfMatchingChars = findListOfListsOfMatchingChars_Char(
