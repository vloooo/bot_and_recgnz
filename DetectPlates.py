...
def detectPlatesInScene(imgOriginalScene):
    ...
    listOfPossibleCharsInScene = validateChars(imgGrayscaleScene, listOfPossibleCharsInScene)
    ...
    listOfListsOfMatchingCharsInScene = DetectChars.findListOfListsOfMatchingChars(listOfPossibleCharsInScene ,imgGrayscaleScene)
    if Main.showSteps == True: # show steps #######################################################
...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):                       # for each contour
        ...
        if checkIfPossibleCharS(possibleChar):                   # if contour is a possible char, note this does not compare to other chars (yet) . . .
...
def checkIfPossibleCharS(possibleChar):
...
def validateChars(img, listOfChars):
    ...
    for i in range(0, len(contours)):  # for each contour
        ...
        if checkIfPossibleCharS(possibleChar):
            for k in listOfChars:
                if abs(k.intCenterX - possibleChar.intCenterX) <= 7 and abs(k.intCenterY - possibleChar.intCenterY) <= 7:
                    listToVld.append(k)
                    # break
